...
export const TECHNIQUE_DEFINITIONS: TechniqueDefinition[] = [
  ...
  },
  {
    ...
    generateSchema: (params) => {
      ...
      const miniSets = params.miniSets || 3;
      return Array(miniSets).fill(reps).join('+');
    },
  },
  {
    name: 'Myo-Reps',
    ...
    generateSchema: (params) => {
      const activation = params.activation || 12;
      const myoReps = params.myoReps || 5;
      const numMyo = params.numMyo || 4;
      return `${activation}+${Array(numMyo).fill(myoReps).join('+')}`;
    },
  },
  {
    name: 'Drop-Set',
    ...
    generateSchema: (params) => {
      const drops = params.drops || 3;
      const repsPerDrop = params.repsPerDrop || 8;
      return Array(drops).fill(repsPerDrop).join('+');
    },
  },
  {
    name: 'Cluster Sets',
    ...
    generateSchema: (params) => {
      const repsPerCluster = params.repsPerCluster || 3;
      const numClusters = params.numClusters || 5;
      return Array(numClusters).fill(repsPerCluster).join('+');
    },
  },
  {
    name: 'Reps Scalare',
    ...
    generateSchema: (params) => {
      const startReps = params.startReps || 12;
      const endReps = params.endReps || 6;
      const step = params.step || 2;
      const reps = [];
      for (let r = startReps; r >= endReps; r -= step) {
        reps.push(r);
      }
      return reps.join('+');
    },
  },
  {
    ...
    generateSchema: (params) => {
      const startReps = params.startReps || 6;
      const endReps = params.endReps || 12;
      const step = params.step || 2;
      const reps = [];
      for (let r = startReps; r <= endReps; r += step) {
        reps.push(r);
  ...
  },
  {
    ...
    generateSchema: (params) => {
      const fullReps = params.fullReps || 8;
      const sets = params.sets || 1;
      return Array(sets).fill(fullReps).join('+');
    },
  },
];
